...
TabPanel = function(config){
  ...
  this.tabWidth = 130;//样式.tabpanel_mover li中width+4
...
TabPanel.prototype = {
  ...
  init : function(){
    ...
    //实例化右键菜单
      this.tabpanel.bind("contextmenu",function(e){
          return false;
      });
    this.tabpanel_right_menu = $('<ul id="ctxMenu" class="contextMenu"></ul>');
      this.tabpanel_right_menu.append("<li action='close' class='ui-helper-clearfix'> <span style='display: block'><span class='text'>关闭当前标签页</span></span></li>");
      this.tabpanel_right_menu.append("<li action='closeOthers' class='ui-helper-clearfix'> <span style='display: block'><span class='text'>关闭其它标签页</span></span></li>");
      this.tabpanel_right_menu.append("<li action='closeLeft' class='ui-helper-clearfix'> <span style='display: block'><span class='text'>关闭左侧标签页</span></span></li>");
      this.tabpanel_right_menu.append("<li action='closeRight' class='ui-helper-clearfix'> <span style='display: block'><span class='text'>关闭右侧标签页</span></span></li>");
      this.tabpanel_right_menu.append("<li action='closeAll' class='ui-helper-clearfix'> <span style='display: block'><span class='text'>关闭所有标签页</span></span></li>");
    this.tabpanel_right_menu.appendTo(this.tabpanel);
    //实例化内容层
  ...
  addTab : function(tabitem){
    ...
    if($('#'+tabitem.id).length>0)
    {
      ...
      ////如果当tabitem的html内容与已经加载了的不同则iframe重新载入html（设备定位到已经打开的图形时无反应）20150819 wdb//
      position = this.getTabPosision(tabitem.id);
      if(this.tabs[position].content.html() !='' && this.tabs[position].content.html() != tabitem.html) {
          this.tabs[position].content.html(tabitem.html);
        }
      ////////////////////////////////////////////////////////////////////////////////////////////////
    }
    ...
    else if(this.scrollFinish)
    {
      ...
      if(tabitem.icon) {
        ...
        if(title.width()>(this.tabWidth-30-wFix)) {
          title.width((this.tabWidth-49-wFix));
          title.attr('title', tabitem.title);
          tab.append('<DIV>...</DIV>');
        }
      }
...
/////////////////添加标签页时为其绑定右键事件 wdb 20150820////
      tab.contextMenu({
			menu : 'ctxMenu',
			onPopup : function(el) {
				var m = $('#ctxMenu');
				m.enableContextMenuItems();
				return true;
			},
			offset: { x: 0, y: 0 }
		}, function(action, el, pos) {
			var c = el.attr('id');//获取点击的标签页id
			var tabNum = tabEntity.getTabsCount();//获取标签数目
			var pageIndex = tabEntity.getTabPosision(c);//根据id获取位置
			switch (action) {
			case 'close':
				tabEntity.kill(pageIndex);//关闭当前标签页
				break;
			case 'closeOthers':
				
				for(var m=tabNum-1;m>=0;m--){
					if(tabEntity.tabs[m].id != c){
						tabEntity.kill(m);
					}
				}
				break;
			case 'closeAll':
				...
				break;
			case 'closeLeft':
				for(var m=pageIndex-1;m>=0;m--){
					tabEntity.kill(m);
				}
				break;
			case 'closeRight':
				for(var m=tabNum-1;m>pageIndex;m--){
					tabEntity.kill(m);
				}
				break;
			}
 ...
 ///////////////////////////////////////////////////////end     
  ...
  show : function(position, notExecuteMoveSee){
    ...
    this.currentTabName = this.getTitle(position);
    //修改保证刷新整图时能够为当前活动标签
    currentId = this.tabs[position].id + "If";
    //此处用于更新最上层
    if(typeof top.updataSvgName == 'function'){
    	top.updataSvgName(this.currentTabName);
    } 
...
////////因下标改变,重新为选项卡绑定右键事件 wdb 20150820////////
      this.tabs[i].tab.contextMenu({
			menu : 'ctxMenu',
			onPopup : function(el) {
				var m = $('#ctxMenu');
				m.enableContextMenuItems();
